...
\begin{draftnote}
Changes on DM-23248 above intentionally do not conform to DMS-REQ-0280, which puts an upper bound of 1 year on the time range of epochs included in templates.
I believe this requirement represents only one way to ensure high-proper-motion stars are dealt with correctly, but utilizing longer time ranges of inputs may be the *only* way to deal with other problems in image subtraction (especially DCR), because those problems are fundamentally limited by available information, not (just) the model used.
...
\begin{draftnote}
Changes on DM-23248 explicitly violates DMS-REQ-0334 (which coadds are retained), removing "Best Seeing Coadds", because DRP isn't actually planning to build or use them internally.
We could produce and serve them anyway to meet the requiement, but I suspect there are much more scientifically productive uses (e.g. postage-stamp coadds) for that part of the storage budget, and I think it's irresponsible to promise to devote resources to something we're not sure will be useful.

Ideally, we would also update the text of DMS-REQ-{0281,0330,0334} to state that DM "will deliver software to produce" these coadds rather than actually producing them.
At least right now, DM has no need to produce them for its internal use, and it's silly to promise to produce something that we are also promising to throw away.
\end{draftnote}
