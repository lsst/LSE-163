...
\item For those that are, an orbit is fitted and a new \SSObject table entry created. \DIASource records are updated to point to the new \SSObject record. \DIAObjects ``orphaned'' by this unlinking are marked as expired (as per Section \ref{sec:repeatability}).
...
The alert processing design relies on the \PPDB that contains the objects and sources detected on difference images. At the very least\footnote{It will also contain exposure and visit metadata, MOPS-specific tables, etc. These are either standard/uncontroversial, implementation-dependent, or less directly relevant for science and therefore not discussed in this document.}, this database will have tables of \DIASources,\dmreq{0269} \DIAObjects,\dmreq{0271} and \SSObjects,\dmreq{0273} populated in the course of nightly and daily difference image and Solar System object association. As these get updated and added to, their updated contents becomes visible (query-able) to users within 24 hours\reqparam{L1PublicT} of the corresponding observation times. In the case of SSObjects, this means 24 hours after orbits have been determined or updated.\dmreq{0312}
...
The time range of the epochs included in a template is TBD, and will be chosen to balance our analysis of high-proper-motion stars (favoring shorter ranges) with the need to correct for DCR and maximize depth (favoring longer ranges).
The numbers and types of coadds used for templates are also TBD.

% Changes on DM-23248 above intentionally do not conform to DMS-REQ-0280, which
% puts an upper bound of 1 year on the time range of epochs included in
% templates.  I believe this requirement represents only one way to ensure
% high-proper-motion stars are dealt with correctly, but utilizing longer
% time ranges of inputs may be the *only* way to deal with other problems
% in image subtraction (especially DCR), because those problems are
% fundamentally limited by available information, not (just) the model used.

% We don't seem to say anything about the availability window of templates.
% Should we?
...
and some \Object measurements (e.g. stellar motion parameters) \emph{may} utilize positions from securely-matched \Sources.}.\dmreq{0034}
    ...
    \item \emph{Object Postprocessing}.  Measurements from all previous steps are combined in catalog-space algorithms.  This includes computing photometric redshifts, fitting proper motion and parallax, and characterizing variability.  These are also used to populate the \Object table.
\end{enumerate}
...
\begin{itemize}
    ...
    To mitigate this effect, we are considering both building per-object postage-stamp coadds or by discretizing the inclusion or rejection of an epoch into the coadd over small predefined (and hence ensuring PSF discontinuities appear only at the well-defined boundaries between these regions).
    ...
    \item RGB color images derived from coadds (utilizing any three of \textit{ugrizy}), intended for visual inspection and EPO \dmreq{0103}.  These will may be stored using lossy compression (e.g. JPEG) and are not intended to be used as an input to algorithmic code
\end{itemize}
...
% Changes on DM-23248 explicitly violates DMS-REQ-0334 (which coadds are
% retained), removing "Best Seeing Coadds", because DRP isn't actually planning
% to build or use them internally.  We could produce and serve them anyway to
% meet the requiement, but I suspect there are much more scientifically
% productive uses (e.g. postage-stamp coadds) for that part of the storage
% budget, and I think it's irresponsible to promise to devote resources to
% something we're not sure will be useful.
%
% Ideally, we would also update the text of DMS-REQ-{0281,0330,0334} to state
% that DM "will deliver software to produce" these coadds rather than actually
% producing them.  At least right now, DM has no need to produce them for its
% internal use, and it's silly to promise to produce something that we are also
% promising to throw away.
