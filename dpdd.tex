...
\textbf{Prompt} (formerly Level 1) data products are computed and released on a roughly nightly timescale, and include images, difference images, catalogs of sources and objects detected in difference images, and catalogs of Solar System objects.
Their primary purpose is to enable rapid follow-up of time-domain events.
\textbf{Data Release} (formerly Level 2) data products are computed during annual processing
campaigns, and include well-calibrated single-epoch images, deep coadds, and catalogs of objects, sources, and forced sources, enabling static sky and precision time-domain science.
The \textbf{Science Platform} will allow for the creation of  \textbf{User Generated} (formerly Level 3) data products and will enable science cases that greatly benefit from co-location of user processing and/or data within the LSST Archive Center.
...
Different data products will generally be stored in different databases. For example, Prompt data products will \oldtext{be}\newtext{include catalogs} stored in a \oldtext{\emph{Level 1 (Prompt) database}} \newtext{Prompt Products Database}. Data Release ``universal cadence'' products will be stored in a \emph{Data Release database}. The products for special programs may be stored in many different databases, depending on the nature of the program.
