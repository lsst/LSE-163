...
\item An alert is issued that includes: the name of the \DB, the timestamp of when this database has been queried to issue this alert, the \DIASource ID, the \SSObject ID or \DIAObject ID\footnote{We guarantee that a receiver will always be able to regenerate the alert contents at any later date using the included timestamps and metadata (IDs and database names).}, name of the \DR and the IDs of nearby \Objects, and the associated science content (centroid, fluxes, low-order lightcurve moments, periods, etc.), \emph{including \oldtext{the full light curves} \newtext{all}} \newtext{\DIASources from the last 12 months\reqparam{diaCharacterizationCutoff} that are linked with the \SSObject or \DIAObject}. \newtext{The science content associated with the \DR \Objects will not be included.} See Section \ref{sec:voEventContents} for a more complete enumeration.
\dmreq{0274}
...
\item The collection of tracklets collected over the past 30 days\footnote{\newtext{The exact time period is a design and computational question discussed in \lsstdoc{LDM-156}.}} is searched for subsets forming \emph{tracks} consistent with being on the same Keplerian orbit around the Sun.
...
\item Science Data:
    ...
    \item \oldtext{All p}\newtext{P}revious \newtext{12 months of} \DIASource records\reqparam{diaCharacterizationCutoff}
    \item \oldtext{A matching \DIAObject} \newtext{Matching \Object IDs} from the latest Data Release, if \oldtext{it} \newtext{they} exist\oldtext{s}, and \oldtext{its} \newtext{12 months of their} \DIASource records
    \end{itemize}
