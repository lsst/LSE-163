...
The number of \VOEvents transmitted to each user per \R{user} \B{visit} will be limited as well (e.g., \R{at least up to $\sim 20$} \B{the equivalent of 20 full-size alert packets} per visit per user\req{numBrokerAlerts}\dmreq{0343}, dynamically throttled depending on load).
Finally, the total number of simultaneous subscribers is likely to be limited\req{numBrokerUsers} -- in case of overwhelming interest, a TAC-like proposal process may be instituted.
