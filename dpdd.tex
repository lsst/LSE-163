...
\annotate{Why is this speculation included in this document?}
...
\annotate{diaSourceThresh seems to be identical to transSNR. We need to agree.}
\DIASources are sources detected on difference images (those with the signal-to-noise ratio $S/N>transSNR$ after correlation with the PSF profile, with $transSNR$\req{transSNR}\req{diaSourceThresh} defined in the SRD and presently set to \transSNR). They represent changes in flux with respect to a deep template. Physically, a \DIASource may be an observation of new astrophysical object that was not present at that position in the template image (for example, an asteroid), or an observation of flux change in an existing source (for example, a variable star). Their flux can be negative (eg., if a source present in the template image reduced its brightness, or moved away). Their shape can be complex (eg., trailed, for a source with proper motion approaching $\sim {\rm deg}/{\rm day}$, or ``dipole-like", if an object's observed position exhibits an offset -- true or apparent -- compared to its position on the template).
...
rerun during Data Release production, see \S~\ref{sec:l1dbreproc}.}. All affected columns will be recomputed, including proper motions, centroids, light curves, etc. \annotate{The requirements need to be updated to match the 12 month cut off. We do not have this value anywhere in DMSR.}\dmreq{0274}
...
This is a table of sources detected at $transSNR \geq \transSNR$ \req{transSNR} on difference images\footnote{This requirement is specified in the LSST SRD.}\req{LSR-REQ-0101} (\DIASources). On average, the LSST SRD expects $\sim 2000$ \DIASources per visit ($\sim 2{\rm M}$ per night; 20,000 per deg$^2$ of the sky per hour). \annotate{Where does SRD say 2000 per visit?}
...
\annotate{apFlux, fpBkgd, spuriousness, dipole model not listed in DMS-REQ-0269. We need to work out whether we are using transSNR or diaSourceThresh. We should really call spuriousness the ``bogosity''.}
...
\annotate{The table here indicates 3 objects for association but DMS-REQ-0271 does not put a limit on the number, simply stating within TBD arcsec.}

\dmreq{0271}\dmreq{0272}\req{diaNearbyObjRadius}
...
\annotate{DMS-REQ-0034 does not explicitly mention DIASources.}
...
\begin{enumerate}
    \item {\em Single Visit Processing}: Raw exposures are reduced to {\em calibrated visit exposures}, and \Sources are independently detected, deblended, and measured on all visits. Their measurements (instrumental fluxes and shapes) are stored in the \Source table.\dmreq{0267}
...
\annotate{The how has to be described in LDM-151.}
        ...
        The six per-band coadds will be kept indefinitely and made available to the users. {\em Their primary purpose is to enable the end-users to apply alternative object characterization algorithms, perform studies of diffuse structures, and for visualization}. \annotate{I don't think the requirements declare which products are to be archived indefinitely.}
               ...
               \item A set of {\em best seeing} coadds. One deep coadd will be created for each of the $ugrizy$ bands, using only the best seeing data (for example, using only the first quartile of the realized seeing distribution). These will be built to assist the deblending process. These coadds will be kept indefinitely and made available to the users. \annotate{The requirements say none of this.}
We will retain and provide sufficient metadata for users to re-create them using Level 3 or other resources. \annotate{These products are archived so this requirement is not critical.}
...
\annotate{This is mentioned in the discussion section of DMS-REQ-0077.}
\\
