...
While the state of the art for these algorithms continues to evolve, our plan for at least DR1 and DR2 is to use the \texttt{Metadetection} algorithm of \citet{2020ApJ...902..138S}.
Additional algorithms may be run as well, as long as their processing time and catalog storage costs are negligible (which is actually the case for a some promising but still unproven methods, e.g. \citet{2016MNRAS.459.4467B}).
...
This is incompatible with our usual desire for algorithms to produce catalogs that map as well as possible to astrophysical reality, especially since theway in which the coadd images are modified represents a slight degredation in signal-to-noise.

As a result, the measurements from \texttt{Metadetection} are best structured as a separate \texttt{ShearObject} table representing a completely distinct set of detections and measurements, with many fewer columns than the main \Object table and approximately 5$\times$ as many rows (for the modified versions of the original coadds).
...
metaStep & int8 & ~ & Enumeration value for which of the five \texttt{Metacalibration} modifications was applied to images prior to all measurements. \\
...
maskFractionCell & float & ~ & Weighted fraction the coadd cell's that were masked, over all visit images input to the coadd.\footnote{This and \texttt{nEpochCell} are the same for all rows with the same \texttt{cellId}, but are expected to be access per-object; they may or may not be normalized into a separate table in the physical schema.}. \\

nEpochCell & int32[ugrizy] & ~ & Number of input visits that contributed to this coadd cell. \\

g1 & float & ~ & Ellipticity from Gaussian-weighted moments, on-axis, measured consistently over central\footnote{Probably $riz$} bands. \\

g2 & float & ~ & Ellipticity from Gaussian-weighted moments, off-axis. \\

gCov & float[3] & ~ & Covariance matrix for (\texttt{g1}, \texttt{g2}). \\

T & float & arcsec$^{2}$ & Gaussian-weighted moments trace (Ixx + Ixy) \\

SNR & float & ~ & Gaussian weighted signal-to-noise ratio. \\

TErr & float & arcsec$^{2}$ & Uncertainty on \texttt{T}.\\

g1PSFMeta & float & ~ & PSF model ellipticity from Gaussian-weighted moments, on-axis, computed from the central-band-averaged metacalibration-modified PSF models.\footnote{This and other PSF quantities are not constant over a cell only because the PSF may depend on the SED of the object.} \\

g2PSFMeta & float & ~ & PSF model ellipticity from Gaussian-weighted moments, off-axis, computed from the central-band-averaged metacalibration-modified PSF models. \\

TPSFMeta & float & arcsec$^{2}$ & Gaussian-weighted moments trace (Ixx + Ixy) of the central-band-averaged metacalibration-modified PSF models. \\

g1PSFOrig & float & ~ & PSF model ellipticity from Gaussian-weighted moments, on-axis, computed from the central-band-averaged original PSF models. \\

g2PSFOrig & float & ~ & PSF model ellipticity from Gaussian-weighted moments, off-axis, computed from the central-band-averaged original PSF models. \\

TPSFOrig & float & arcsec$^{2}$ & Gaussian-weighted moments trace (Ixx + Ixy) of the central-band-averaged original PSF models. \\

stdFlux & float[ugrizy] & nJy & PSF-corrected aperture fluxes, measured using an approach similar to the ``standard color'' in the main \Object table.\footnote{This is not expected to be a good estimate of total flux, but \emph{some} estimate of total flux is useful, so this is still reported as fluxes, as opposed to the colors in \Object's \texttt{stdColor}.} \\
