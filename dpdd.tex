...
The \emph{broad}, \emph{high-level}, requirements for LSST Data Products are given by the \emph{LSST Science Requirements Document}\footnote{LSST Document Handle LPM-17 \cite{LPM-17}, available at \url{http://ls.st/srd}} (SRD). This document lays out the \emph{specifics} of what the data products will comprise of, how those data will be generated, and when. It serves to inform the flow-down from the LSST SRD through the \emph{LSST System Requirements Document} (the LSR; LSE-29 \cite{LSE-29}) and the \emph{LSST Observatory System Specifications} (OSS; LSE-30 \cite{LSE-30}), to the \emph{LSST Data Management System Requirements} (DMSR; LSE-61 \cite{LSE-61}), the UML model, and the database schema (LDM-153 \cite{LDM-153}).
...
\item The collection of tracklets collected over the past 30 days\footnote{\B{The exact time period is a design and computational question discussed in LDM-156\nocite{LDM-156}.}} is searched for subsets forming \emph{tracks} consistent with being on the same Keplerian orbit around the Sun.
...
In course of Level 2 processing, multiple classes and numerous of coadds will be created\footnote{\B{See LDM-151 \cite{LDM-151} for more details.}}:
    \begin{itemize}
...
document\footnote{See \url{http://ls.st/LDM-151}} LDM-151 \cite{LDM-151}.
