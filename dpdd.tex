...
measurements (proper motions, centroids, light curves, etc.). \req{diaCharacterizationCutoff}
...
Raw and \B{P}\R{p}rocessed \B{V}\R{v}isit \B{I}\R{i}mages will be made available for download no later than 24 hours\req{L1PublicT} from the end of visit acquisition.
...
We emphasize that this LSST-provided capability will be limited, and is \emph{not} intended to satisfy the wide variety of use cases that a full-fledged public Event Broker could. For example, we do not plan to provide any \B{\emph{exclusive}} classification \B{to a unique category of object} (eg., ``is the light curve consistent with an RR Lyra?'', or ``a Type Ia SN?''), \B{although pre-defined filters will be provided\dmreq{0348} with potentially highly overlapping selections, that provide good completeness but perhaps only very modest purity for a small number of object types of common interest}. No information beyond what is contained in the \VOEvent packet will be available to user-defined filters (eg., no cross-matches to other catalogs). The complexity and run time of user defined filters will be limited by available resources. Execution latency will not be guaranteed. The number of \VOEvents transmitted to each user per \R{user} \B{visit} will be limited as well (eg., \R{at least up to $\sim 20$} \B{the equivalent of 20 full-size alert packets} per visit per user\req{numBrokerAlerts}\dmreq{0343}, dynamically throttled depending on load). Finally, the total number of simultaneous subscribers is likely to be limited\req{numBrokerUsers} -- in case of overwhelming interest, a TAC-like proposal process may be instituted.
...
In course of Level 2 processing, multiple classes and numerous of coadds will be created\footnote{\B{See LDM-151\nocite{LDM-151} for more details.}}:
    \begin{itemize}
...
Processing for special programs will make use of the same software stack and computing capabilities as the processing for universal cadence.\dmreq{0320} The programs are expected to use no more than $\sim$10\% of computational and storage capacity of the LSST data processing cluster. When special products include time domain event alerts, their processing shall generally be subject to the same latency requirements as Level 1 data products.\dmreq{0321}\dmreq{0344}\req{OTT1}
