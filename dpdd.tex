...
The three tables presented below capture the high-level information that will be available in the database for each Solar System Object (\SSObject) and for each observation of a Solar System Object (\SSSource). They are primarily meant as {\bf guidance} to the LSST team when developing the actual schema. {\bf The details and the schema realization are nearly certain to differ from what's given below}. The implementation will be maintained in the catalog repository at \url{https://github.com/lsst/cat}, as well as linked at \url{http://ls.st/ssp}.

\dmreq{0273}

\begin{schema}{{\tt MPCORB} Table}{{\tt MPCORB} Table}{tbl:mpcorbTable}
	
	ssObjectId & uint64 & ~ & Unique identifier (internal). \\
	
	mpcDesignation & string[8] & ~ & Unique MPC designation. \\
...
MOID & float[2] & AU & Minimum orbit intersection distance with Earth\footnote{\url{http://www2.lowell.edu/users/elgb/moid.html}} \\

moidLon & double & degrees & MOID longitude. \\

H & float[6] & mag & Mean absolute magnitude, per band \citep[][magnitude-phase system]{2010Icar..209..542M}. \\

$\mathrm{G_{12}}$ & float[6] & mag & $G_{12}$ slope parameter, per band \citep[][magnitude-phase system]{2010Icar..209..542M}. \\

hErr & float[6] & mag & Uncertainty of H estimate.\\

g12Err & float[6] & mag & Uncertainty of $G_{12}$ estimate. \\
