...
\begin{itemize}
    ...
    \item Removed \texttt{xMag, xMagErr, xAmplitude, xPeriod} columns as they were not clearly defined (eg., w/o phase correction, do they make sense?). These can be recovered by querying the \DIASource table for magnitudes. Deriving phase-corrected light curves is left as a User Generated task.
...
\emph{The Science Platform} is  envisioned to enable science cases that would greatly benefit from co-location of user processing and/or data within the LSST Archive Center. The high-level requirement for the Science Platform  is established in \S~3.5 of the LSST \SRD.
\ossreq{0140}

The science platform is described in in \citeds{LSE-319}
with more formal requirements in \citeds{LDM-554} and the design   is in \citeds{LDM-542}.

Users will have access to the platform via logon granting them:
\begin{enumerate}
    \item Access to the Data Products and associated storage resources
    \item processing resources, and
    \item the LSST tools and processing  framework
\end{enumerate}
%
\subsection{User Generated Data Products and Associated Storage Resources}
...
For catalog data products, there is an expectation that they can be ``federated'' with the Prompt and Data Release catalogs to enable analyses combining them.  Essentially this means that either the user-supplied tables include keys from the Prompt/Data Release catalogs that can be used for key-equality-based joins with them (example: a table of custom photometric redshifts for galaxies, with a column of object IDs that can be joined to the Data Release Object catalog), or that there are columns that can be used for spatial (or temporal, or analogous) joins against Prompt/Data Release tables. The latter implies that such User Generated product table's columns must be in the same coordinate system and units as the corresponding Prompt/Data Release columns.
\dmreq{0123}
\dmreq{0124}

There is no requirement that User Generated data products are derived from Prompt or Data Release products other than that they be joinable with them.  For instance, a user might have a catalog of radio sources that they might want to bring into federation with the LSST catalogs. That can be thought of as a User Generated Data Product as long as they have ``LSST-ized'' it by ensuring compatibility of coordinate, time, measurement systems, etc.  Nevertheless, we do expect the majority of User Generated data products to be derived from processed LSST data.

There could also be User Generated image data products; for example, user-generated coadds with special selection criteria or stacking algorithms (eg. the so-called shift \& stack algorithm for detecting moving objects).
\dmreq{0127}

Any User Generated data products may have access controls associated with it, restricting read access to just the owner, to a list of people, to a named group of people, or allowing open access.\dmreq{0340}
...
It is to be decided whether users will be required to provide the code and/or documentation behind their User Generated data products, or whether the SAC may include the availability of this supporting information in its prioritization.  Obviously if a user intends to make a User Generated data products public or publish it to a group it will be more important that supporting information be available.
