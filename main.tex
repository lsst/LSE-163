...
{\em \bf (*** DRAFT ***)}}
\author{
    Mario Juri\'c\footnote{Corresponding author, \textless\href{mailto:mjuric@lsst.org}{mjuric@lsst.org}\textgreater.},
    R. H. Lupton, T. Axelrod, G.P. Dubois-Felsmann, \\    
    \v{Z}. Ivezi\'c, A.C. Becker, J. Becla,  M. Freemon, J. Kantor, \\
    K-T Lim, D. Shaw, M. Strauss, {\em and} J.A. Tyson \vspace{1.2ex} \\
...
\begin{abstract}
This document describes the data products to be produced by the Large Synoptic Survey Telescope (LSST). It also desribes the capabilities and resources that will be made available to end-users at the LSST data center (also known as "Level 3").
...
Though under strict change control\footnote{LSST Docushare handle for this document is {\tt LSE-163}.}, this is a {\bf \em living document}. LSST will undergo a period of construction  and commissioning lasting seven years, followed by a decade of survey operations. The designs and plans presented here will be periodically reviewed and updated, to ensure the continued scientific adequacy of LSST Data Products.

\clearpage

\section{Introduction and General Considerations}

\subsection{Large Synoptic Survey Telescope}
...
Recognizing the diversity of astronomical community needs, and the need for specialized processing not part of the automatically generated Level 1 and 2 products, LSST will devote 10\% of its data management system capabilities to enabling the creation, use, and federation of {\bf Level 3} (user-created) data products. Level 3 capabilities will enable science cases that greatly benefit from co-location of user processing and/or data within the LSST Archive Center. The high-level requirement for Level 3 is established in \S~3.5 of the LSST SRD. Their details are discussed in \S~\ref{sec:level3} of this document.

\vspace{1em}

LSST Survey Specifications (LSST SRD, \S~3.4) prescribe that 90\% of LSST observing time be spent executing the so-called ``universal cadence". These observations will result in Level 1 and 2 data products described so far in this document. The remaining 10\% of observing time will be devoted to special programs, obtaining improved coverage of interesting regions of observational parameter space. Examples include very deep ($r \sim 26$, per exposure) observations, observations with very short revisit times ($\sim$1 minute), and observations of ``special" regions such as the Ecliptic, Galactic plane, and the Large and Small Magellanic Clouds. The data products generated by these {\bf special programs} will use the same processing software, possess the general characteristics of Level 1 and 2 data products, but may be performed on somewhat different cadence. They will be discussed in \S~\ref{sec:specialProgs}.

\subsection{General Considerations}

Most LSST data products will either consist of images or catalogs. The catalogs will be stored and offered to the users as {\em relational databases} which they will be able to query. This approach was shown to work well by prior large surveys, most notably the Sloan Digital Sky Survey (SDSS).

Various data products may be stored in different databases (eg., Level 1 data products will be stored in a ``Level 1" database, Level 2 ``universal cadence" products will be stored in a different database, etc.). Nevertheless, all these databases will follow certain naming and other conventions. We discuss these in the subsections to follow, before dwelving into the details of the data products themselves.

\subsubsection{Estimator and Naming Conventions}

In all catalogs (tables), we will employ a convention where estimates of standard errors have the suffix {\tt Err}, while the estimates of inherent widths of distribution (or functions in general) have the suffix {\tt Sigma}\footnote{Given $N$ measurements, standard errors scale as $N^{-1/2}$, while widths remain constant.}. The latter are defined as the square roots of the second moment about the quoted value of the quantity at hand.

Unless noted otherwise, maximum likelihood values will be quoted for all fitted parameters (measurements). Together with covariances, these let the end-user apply whatever prior they deem appropriate when computing posteriors\footnote{There's a tacit assumption that a Gaussian is a reasonably good description of the likelihood surface around the ML peak.}. Where appropriate, multiple independent samples from the likelihood may be provided.
...
We quote fluxes in units of ``maggie''. A maggie, as introduced by SDSS, is a linear measure of flux. It is defuned so that an object having a flux of one maggie (integrated over the bandpass) has an AB magnitude of zero:
\begin{equation}
...
To reduce the likelihood for confusion, all IDs shall be unique across databases and database versions, other than those corresponding to uniquely identifiable physical entities (i.e., IDs of exposures).
...
We require that queries executed at a known point in time against any project-delivered database be repeatable at a later date. This promotes the reproducibility of science derived from LSST data. It is of special importance for Level 1 catalogs (\S~\ref{sec:level1}), that will change on a nightly basis.

The exact implementation of this requirement is under consideration by the LSST Data Management database team. One possibility may be to make the key tables (nearly) append-only, with each row having two timestamps -- createdTai and deletedTai, so that queries may be limited through a \code{WHERE} clause:
%
\begin{quote}
...
LSST does {\em not} plan to build or deploy algorithms specifically designed for crowded field photometry. {\em \bf Processing these areas using specialist crowded field photometry codes is left to the users as a Level 3 task (see \S~\ref{sec:level3})}.
...
Over 10 years of operations, LSST will produce eleven data releases: two for the first year of data, and one every subsequent year. Each data release will include reprocessing of all data from the start of the survey, up to the cutoff date for that release.
...
\begin{itemize}
    \item {Is our approach to crowded field photometry satisfactory?} Is there another, given the budget and schedule constraints?

    \item {\em Are we going to characterize isophote twists (bars, etc)?} This is a question of how far should (and can) we go with "large" galaxy characterization. Measures like isophote twists, Gini coefficients, etc. This is now considered to be Level 3, but it's likely at least a portion of it belongs to Level 2 -- assuming we're given a set of well-defined morphology measures. Input from the galaxies (and strong lensing?) Collaborations would be useful.
...
LSST Survey Specifications (LSST SRD, \S~3.4) specify that 90\% of LSST observing time will be spend executing the so-called ``universal cadence". These observations will result in Level 1 and 2 data products described so far in this document.
...
The details of these special programs or micro surveys are not yet defined\footnote{The initial complement is expected to be defined and selected no later than Science Verification}. Consequently, the specifics of their data products are left undefined at this time. Instead, we just specify the {\em constraints} on these data products, given the adopted Level 1/2/3 architecture. It is understood that no special program will be selected that does not fit these constraints\footnote{Or comes with additional, external, funding, capabilities, and/or expertize.}. These constraints allow us to size and construct the data management system, with no need to knowing the exact definition of these programs far in advance.
\\
