...
This single-visit image processed by the ISR is called "calibrated
...
\B{The main LSST data products are illustrated in Figure~\ref{fig:Detail0}.} 
...
\subsection{Conceptual Design of Science Pipelines} 
...
Further details about the pipeline design and implementation are available from the LSST document LDM-151. 
}
...
%    \hskip -0.2in
    ...
    \vskip -0.1in 
    \caption{Illustration of the conceptual algorithm design for Moving Object Processing System. \label{fig:Pipe8}}    
\end{figure}
...
\B{Some \DIASources will be caused by background fluctuations; with $transSNR \geq \transSNR$, 
...
(eg., binned detection algorithm from SDSS).}  Each footprint may have one or more {\em peaks}, and the collection of these peaks (and their membership in the footprints) are the output of this stage. 
    ...
    \item \B{{\em Coadd object characterization}.  Object properties such as adaptive moments and aperture fluxes will be measured on the coadds.  These will be stored in additional columns in the \Object table.  Models fit in multi-epoch object characterization will also be fit first to the coadds to initialize the more expensive multi-epoch fitting.} 
...
\item {\em Variability characterization}. Two groups of parameters will be provided, designed to characterize periodic and aperiodic variability features (Richards et al. 2011). \B{These parameters will be computed using flux measurements performed on difference images.}
...
\item If the original code was written in another language or another data processing framework, the project will 
\R{have the resources to rewrite} \B{consider rewriting} it to required LSST standards.
